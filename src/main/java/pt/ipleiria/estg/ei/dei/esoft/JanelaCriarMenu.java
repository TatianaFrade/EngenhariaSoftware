...
public class JanelaCriarMenu extends JDialog {
    ...
    public JanelaCriarMenu(Window parent, Runnable onClose) {
        super(parent, "Criar Novo Menu", ModalityType.APPLICATION_MODAL);
        ...
        checkAtivo = new JCheckBox("Menu Ativo", true);
        ...
        JButton btnGuardar = new JButton("💾 Criar");
        btnGuardar.addActionListener(e -> {
            try {
                Menu novoMenu = new Menu();
                novoMenu.setId(PersistenciaService.gerarNovoIdMenu());
                novoMenu.setNome(campoNome.getText().trim());
                novoMenu.setDescricao(campoDescricao.getText().trim());
                novoMenu.setAtivo(checkAtivo.isSelected());

                Filme filmeSelecionado = (Filme) comboFilmes.getSelectedItem();
                novoMenu.setFilme(filmeSelecionado != null ? filmeSelecionado.getNome() : "");
                ...
                novoMenu.setItens(novosItens);
                novoMenu.setPreco(calcularPrecoTotal());

                PersistenciaService.adicionarMenu(novoMenu);

                JOptionPane.showMessageDialog(this, "Menu criado com sucesso.");
                dispose();

                if (onClose != null) onClose.run();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Erro ao criar menu: " + ex.getMessage());
            }
        });
